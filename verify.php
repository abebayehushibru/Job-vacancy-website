...
if(isset($_POST['verify'])){
    include_once('verifyprocess.php');
}

echo "<span>we've sent a verfiction to you phone number </span><span>";
$_SESSION['adminphno'] = "0964799523";
$phno = str_split($_SESSION['adminphno']);
$x=strlen($_SESSION['adminphno']);
$i=0;
while($i<$x){
    if($i<4){
        echo"$phno[$i]"; 
...
?>
                <div>
                    <input type="number" name="verifycode" maxlength="6" minlength="6" placeholder="enter verification code" class="phonenumber">
                    <span></span>
                </div>
           
           <div> <button class="submit" name="verify"> verify </button></div>
